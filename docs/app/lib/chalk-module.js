...
function buildChalkMaterials(baseColor, seed, glyph) {
  ...
  return faces.map((idx)=>{
    ...
    return new THREE.MeshStandardMaterial({
      ...
      envMapIntensity: 0,
      fog: false,
      // ОТКАЧЕНО (заход 65): depthWrite:false здесь пробовали как фикс
      // асимметричной прозрачности (заход 63) — гипотеза не подтвердилась
      // визуальной проверкой: проблема с прозрачностью осталась почти
      // везде, кроме одного примера, а вдобавок появился НОВЫЙ баг —
      // кубики с более поздним номером слота стали рисоваться поверх
      // более ранних вне зависимости от реального расположения в
      // пространстве (депth-buffer перестал быть источником истины для
      // порядка отрисовки). Настоящая причина прозрачности пока не
      // найдена — ищем дальше через реальные числа из работающей
      // страницы (DevTools), не через гипотезы по коду.
    });
  });
}
