...
function buildChalkMaterials(baseColor, seed, glyph) {
  ...
  return faces.map((idx)=>{
    ...
    return new THREE.MeshStandardMaterial({
      ...
      envMapIntensity: 0,
      fog: false,
      // transparent:true (ставится в buildMatSet) без depthWrite:false — известная
      // ловушка Three.js: полупрозрачный объект всё равно пишет в буфер глубины
      // как непрозрачный, из-за чего соседние притенённые кубики перекрывают
      // друг друга непредсказуемо (по порядку отрисовки, не по реальной
      // прозрачности) — визуально часть кубиков выглядит менее прозрачной, чем
      // должна. depthWrite:false — стандартное решение для объектов, которые
      // плавно меняют прозрачность (не «стекло», сквозь которое видно другое).
      depthWrite: false,
    });
  });
}
