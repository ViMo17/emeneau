...
function paintGlyph(cv, ch) {
  ...
  // Данда («।», маркер паузы avasāna, rule31/suhārt) — НЕ буква, обычный
  // текстовый рендер через fillText рисовал бы её как короткий штрих на
  // высоту строчной буквы — визуально путается с заглавной I/строчной l
  // (прямая правка пользователя после живой проверки). Рисуем явной
  // вертикальной чертой почти во всю грань, а не через шрифт.
  if (ch === '।') {
    const w = sz * 0.055;
    ctx.fillRect(sz / 2 - w / 2, sz * 0.06, w, sz * 0.88);
    ctx.restore();
    return;
  }
