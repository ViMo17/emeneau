...
export const TRANSFORM_KIND = {
  // Звонкость внутри варги (k↔g, d↔t — 1-й член ↔ 3-й, 2-й ↔ 4-й) — 180°,
  // ПРОТИВОЛЕЖАЩАЯ грань. «5 граней кубика = 5 членов варги» (фасад=1-й,
  // по часовой стрелке=2-й/придых., противолежащие=3-й/4-й звонкие,
  // верх=5-й/носовой — геометрическая конвенция, зарезервирована прямым
  // решением пользователя). НЕ покрывает придыхание (см. aspirationPair
  // ниже — другая грань, другой угол) — раньше ошибочно считалось той же
  // категорией, что и привело к неверному углу у правила 10 (было 180°,
  // должно быть 90°).
  vargaPair: { spinTurns: 0.5, signal: 'blank' },
  // Придыхание внутри варги (ch↔c — 1-й член ↔ 2-й, «по часовой стрелке»)
  // — 90°, СОСЕДНЯЯ грань (idx0, не противолежащая idx5 — см.
  // buildOpposingFaceMaterials/applyTransform). Та же «тяжесть» вращения
  // (1800мс/bounceH 0.16), что у vargaPair — оба случая читаются как
  // одна и та же категория «парная замена внутри варги», просто по
  // разным осям/углам.
  aspirationPair: { spinTurns: 0.25, signal: 'blank' },
  // «Любая ↔ назальная» ВНУТРИ одной варги (member 1-4 ↔ 5-й, «верх») —
  // ОТКРЫТО, не построено ни разу: по геометрической конвенции выше это
  // отдельная ось (наклон к верхней грани, не спин вокруг вертикали) —
  // не путать с m→n (rule11), там носовой ОДНОЙ варги переходит в носовой
  // ДРУГОЙ варги целиком (assimToNeighbor ниже, не эта категория —
  // прямое решение пользователя: у нас нет отдельного приёма для
  // носовых, каждый носовой принадлежит своей варге, переход между
  // варгами — просто обычный оборот).
  //
  // Общая ассимиляция под соседа (буква вне всех варг — h/ṃ/ḥ — целиком
  // меняет место образования; ТАКЖЕ покрывает переход носового ОДНОЙ
  // варги в носовой ДРУГОЙ, см. выше) — 360°, обычный оборот, нейтральная
  // грань.
