...
export function mountSlotExample(container, data, opts = {}) {
  ...
  // монтирования). apply* пишут флаги (`op._began`, `op._revealed`,
  ...
  function updateSteps(elapsed) {
    ...
    if (!curStep._virtual && !curStep._reveal && idx !== _lastStepIdx) {
      ...
      // авторский шаг (в т.ч. переход грамматика→правило) — начало нового
      // шага само по себе нуждается в отдельном визуальном сигнале, не
      // только его завершение (см. CLAUDE.md, Часть 2). На _reveal
      // (пауза-проявление между шагами) и _virtual (финальный хвост) не
      // срабатывает — там нет своего чипа, сигналом служит само
      // проявление, отдельная вспышка не нужна.
  ...
  // Vector3.project через camera), не подбор на глаз. РЕАЛЬНЫЙ НАЙДЕННЫЙ
  // БАГ (живая проверка пользователя в самом приложении, не на тестовом
  // полигоне): 3.6 было откалибровано только для «обычного (широкого)
  // окна» (aspect≥2, где computeFitFov держится near-baseFov) — реальная
  // панель `.eff-stage` в приложении часто УЖЕ у него (aspect~1.5-1.8,
  // растёт camera.fov через fovForWidth) — на такой пропорции 3.6
  // проецируется ВНУТРЬ кадра (проверено численно: pixelY от +24 до +76
  // на реалистичных размерах панели), верхняя строка кубиков видна.
  // 5.0 остаётся скрытым (pixelY<0) на всём проверенном диапазоне —
  // от узкого тестового полигона (900x440) до заметно более квадратной
  // панели (1150x620). Затрагивает ВСЕ примеры (общая константа), не
  // только rule50 — эффект чисто позиционный (старт чуть дальше от
  // кадра), fallDur не меняется, длительность падения та же.
  const FALL_START_Y = 5.0;
  ...
  function renderAtElapsed(elapsed) {
    ...
    ops.forEach(op => {
      ...
      else if (op.type === 'resist') applyResist(op, elapsed, ctx);
