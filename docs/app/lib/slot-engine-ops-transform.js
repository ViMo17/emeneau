...
export function applyTransform(op, elapsed, ctx) {
    ...
    if (!op._landed) {
      // clearanceY — ЭКСПЕРИМЕНТАЛЬНАЯ альтернатива clearanceZ (см. ниже),
      // добавлена ТОЛЬКО для живого A/B сравнения по прямому вопросу
      // пользователя («вращение задевает соседей — может быть, поднимать
      // кубик вверх?») — не решение, не замена дефолтного пути. Тот же
      // колокол sin(t·π), что и bounceH — просто складывается с ним (та же
      // ось), поднимая кубик заметно выше обычного подскока на время
      // активного вращения. Дефолт 0 — ни один существующий пример его не
      // передаёт, поведение не меняется.
      cube.mesh.position.y = Math.sin(t * Math.PI) * (bounceH + (op.clearanceY ?? 0));
