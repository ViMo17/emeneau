...
export function applyTransform(op, elapsed, ctx) {
    ...
    // (из-за ускорения easeOutCubic в начале движения). При spinTurns:2
    // (вриддхи, 720°) ТА ЖЕ доля 0.15 от ВДВОЕ БОЛЬШЕГО пути даёт ~278° —
    // грань к этому моменту уже почти довернулась обратно к зрителю,
    // подмена случалась впритык к моменту, когда старая буква снова
    // читаема, а после подмены до её повторного появления в кадре (~360°)
    // — заметная пауза (прямая находка пользователя по экрану: старая
    // буква видна почти до конца первого оборота, новая — только после
    // ~450°). Порог переведён в ГРАДУСЫ поворота (не в долю времени) —
    // подмена происходит на первой же скрытой фазе при любой длительности
    // оборота, не только при 360°. Порог — 130°, не ровно 90° (первая
    // «геометрически ровная» четверть): при наклонной камере движка
    // (camBase не строго анфас) ровно 90° оказалось слишком близко к
    // границе видимости — пользователь заметила мигание новой буквы ДО
    // официального появления из-за поворота именно на гунации (360°),
    // где раньше (t=0.15→~139°) мигания не было. 130° выбрано так, чтобы
    // для гунации получился почти тот же угол, что и раньше (139°→~130°,
    // проверенный практикой), и при этом для вриддхи (720°) оставалось
    // ГОРАЗДО раньше первого возврата грани (130°<<360°) — тот же принцип,
    // не два разных случая.
    const rotatedDeg = easeOutCubic(t) * 360 * Math.abs(spinTurns);
    if (!landsOnOppositeFace && !op._swapped && rotatedDeg >= 130) {
      op._swapped = true;
      const newColor = op.toColor ?? colorFor(op.toGlyph);
...
export function applyElide(op, elapsed, ctx) {
    ...
    // отстойника (-1.1). ПРАВКА по прямому запросу пользователя
    // («симметрично верху» split — немного сбоку, на расстоянии около
    // 1 кубика вниз): y=-1.1 (верхний край отстойника САМ, не его центр
    // на кубик глубже — «около 1 кубика», не полтора), x увеличен с -0.2
    // до -0.45 (заметнее «немного сбоку», раньше почти не читалось).
    // Проверено THREE.Vector3.project — на новой глубине запас даже
    // БОЛЬШЕ прежнего (NDC y=-0.486, было -0.654, край кадра −1.0) — сдвиг
    // ближе к строю может только улучшить запас, не ухудшить его.
    const holdOffset = op.holdOffset ?? { x: -0.45, y: -1.1, z: -0.4 };
