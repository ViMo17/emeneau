...
export function mountSlotExample(container, data, opts = {}) {
  ...
  function applyElide(op, elapsed) {
    ...
    // ИСПРАВЛЕНО (заход 38, «S находится прямо под тенью А»): x:-0.6 из
    // захода 34 был логической ошибкой — «отступает от DH» означало
    // движение ВЛЕВО, но ā сидит именно слева, и зазор до неё сокращался
    // до 0.6 (меньше диаметра тени, 1.76 — отсюда наложение). x:-0.2 —
    // зазор до ā ~1.0, снос по-прежнему не строго вертикальный, но не
    // задевает соседку.
    const holdOffset = op.holdOffset ?? { x: -0.2, y: -1.4, z: -0.4 };
    const basePos = new THREE.Vector3(slotX(op.at), 0, 0);
    const holdPos = basePos.clone().add(new THREE.Vector3(holdOffset.x, holdOffset.y, holdOffset.z));
