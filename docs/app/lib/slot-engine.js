...
// ДОБАВЛЕНО (заход 54, проактивная оптимизация — не только когда прямо
// просят). Раньше сочетание spinTurns+signal для каждой категории
// transform (см. реестр эталонных эффектов, algoritm-agnayas-shablon.md)
// повторялось вручную числами в данных каждого примера — источник реальной
// ошибки (заход 46: spinTurns перепутан с гунацией для парной замены).
// Теперь сочетание существует ОДИН раз здесь, данные примера ссылаются на
// него по имени: { type:'transform', ..., ...TRANSFORM_KIND.vargaPair }.
// guna — не отдельный пресет: дефолты движка (spinTurns:1, signal не
// 'blank') УЖЕ и есть гунация, передавать через пресет нечего (agnayas
// как был без явных spinTurns/signal, так и остаётся — обратная
// совместимость, ничего не сломано).
...
// classifyFor ниже для i/u-рядов). ИСПРАВЛЕНО (заход 49, «не только
// придыхательные, ещё дифтонги» — та же ошибка, что уже ловила с dh, только
// на этот раз не проявилась бы сразу: вриддхи ещё ни разу не строили, но
// сломалась бы гарантированно на первом же примере). Нужен явный список,
// не длина строки — иначе многобуквенный сжатый блок (заход 48) и обычная
...
export function colorFor(tr) {
  ...
  // MULTI_CHAR_PHONEMES выше). Без этого исключения кубики dh/kh/gh/ai/au
...
// ИСПРАВЛЕНО (заход 52, «A слева и справа с разной прозрачностью» —
// найдено сравнением одной и той же буквы в двух позициях, реальный
// механический баг, не восприятие цвета). Раньше проявление («все видны»)
// вставлялось при ЛЮБОМ зазоре между шагами — даже если activeSlots у
// соседних шагов ОДИНАКОВЫ (цепочка правил, тот же состав участников, как
// у taddhiraṇyam: оба шага держат [2,4]). Проявление семантически
// означает «состав участников меняется» — если он не меняется, реальная
// причина показывать его нет, а при коротком зазоре и широкой раскладке
// (много кубиков → большой суммарный сдвиг REVEAL_STAGGER) волна ещё и не
// успевала докатиться до дальнего края за отведённое время, обрывалась на
...
*/
// ПЕРЕНЕСЕНО НА УРОВЕНЬ МОДУЛЯ (заход 56, Стадия 2 профессионализации).
// Раньше была вложена внутрь mountSlotExample — физически не тестировалась
// напрямую, только копированием (см. историю сессии, десятки одноразовых
// симуляций в /tmp). Единственная внешняя зависимость — cubes (карта
// слот→кубик) — теперь явный параметр через объект контекста `ctx`, не
// захват через замыкание. Образец ЕДИНОЙ сигнатуры (op, elapsed, ctx) для
// ВСЕХ apply*-функций — остальные 24 переносятся в следующих заходах по
// той же схеме, не каждая по-своему.
//
...
// правило 3»). Три явные фазы: (1) кубик получает импульс — материал
// сразу переключается на matsSignal (серебро, СТАРАЯ буква ещё видна —
// понятно, КТО меняется); (2) на ~15% оборота буква меняется, но материал
// остаётся серебряным — уже пересобранный под НОВУЮ букву (см. regenMats)
// — кубик «стал E», но ещё серебряный, переход цветом не завершён;
// (3) в момент приземления (t=1, тот же кадр, что и сброс поворота/
// позиции — не отдельная пауза) — переключение на matsMain, истинный
// цвет столбца.
...
export function mountSlotExample(container, data, opts = {}) {
  ...
  authoredSteps.forEach((step, i) => {
    ...
    // ИСПРАВЛЕНО (заход 51, «не подбирай новые цвета — стандартизируй»).
    // Было: цвет зависел от step.kind (grammar→охра, rule→цвет тира),
    // primary добавлял только размер — вспомогательное 'rule' в цепочке
    // (например, правило 71 внутри примера про правило 70) получало ТОТ ЖЕ
    // цвет, что и главное, просто мельче. По прямому указанию — цвет
    // определяется ТОЛЬКО тем, главный ли шаг (primary), не его kind:
    // главный — цвет своего тира (is-rule+is-primary, крупно); ВСЁ
    // остальное — грамматика И любое вспомогательное/цепочное правило —
    // один и тот же уже стандартизированный охровый тон is-grammar
    // (#CDA84E), мелко. Не новый цвет — переиспользование существующего.
    // Текст подписи («Правило N» / «Грамматика») по-прежнему берётся из
    // step.kind отдельно, независимо от того, каким классом это красится.
  ...
  // Объект контекста для функций, перенесённых на уровень модуля (заход 56+,
